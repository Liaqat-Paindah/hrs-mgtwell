...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Get the <p> element by its ID and set its text content
    const dateElement = document.getElementById('now');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let startTime = null;
        ...
        function setCurrentDateTime() {
            const now = new Date();
            const date = now.toISOString().split('T')[0]; // Gets the date in YYYY-MM-DD format
            const time = now.toTimeString().split(' ')[0].slice(0, 5); // Gets the time in HH:MM format
